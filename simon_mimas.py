...
from litex_boards.targets.seven import SevenSegmentDisplay

# CRG ----------------------------------------------------------------------------------------------

class SevenSegmentRegister(Module, AutoCSR):
    def __init__(self, pads, sys_clk_freq):
        self.pads = pads
        self.value = CSRStorage(16)
        self.submodules.display = SevenSegmentDisplay(sys_clk_freq)
        self.comb += [
            pads.digits.eq(~self.display.o_digits),
            pads.segments.eq(~self.display.o_segments)
        ]
...
class BaseSoC(SoCCore):
    def __init__(self, sys_clk_freq=int(100e6), with_ethernet=False, **kwargs):
        ...
        self.submodules.seven_segment = SevenSegmentRegister(pads = platform.request("seven_seg"),
                                                             sys_clk_freq = sys_clk_freq)
